...
def validate_transactions(transactions_df, stores_df, products_df):
    ...
    for field in critical_fields:
        ...
        if not invalid_df.empty:
            ...
            delete_this=invalid_df.index.to_list()
    ...
    # get list of index(es) to delete
    delete_this.extend(invalid_df.index)
    delete_this.extend(duplicate_df.index)
    delete_this.extend(invalid_ts_df.index)
    delete_this.extend(invalid_store_df.index)
    delete_this.extend(invalid_product_df.index)
    delete_this.extend(negative_qty_df.index)
    delete_this.extend(negative_price_df.index)
    
    # create clean df and order by transaction_id
    working_df = working_df.drop(delete_this)
    ...
    count = quarantine_df['transaction_id'].nunique()

    # log counts
    logger.info(f"Total clean records: {len(working_df)}")
    logger.info(f"Total bad records: {count}")
    logger.info(f"Total Issues Found: {len(quarantine_df)}")
