...
def validate_transactions(transactions_df, stores_df, products_df):
    ...
    for field in critical_fields:
        ...
        invalid_df = working_df[
            working_df[field].isna() |
            (working_df[field].astype(str).str.strip() == "")
        ].copy()
    ...
    duplicate_df = working_df[
        working_df.duplicated(subset=["transaction_id"], keep=False)
    ].copy()
    ...
    invalid_store_df = working_df[
        ~working_df["store_id"].isin(valid_store_ids)
    ].copy()
    ...
    invalid_product_df = working_df[
        ~working_df["product_sku"].isin(valid_skus)
    ].copy()
    ...
    negative_price_df = working_df[
        working_df["unit_price"] <= 0
    ].copy()
